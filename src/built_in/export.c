...
int	ft_export(t_minishell *minishell, int fd, char **args)
{
	...
	while (args[i])
	{
		if (ft_is_valid_key_env(args[i]) == 1 ||
			ft_is_only(args[i], ft_isdigit) ||  
			(args[i][0] == '=' && !args[i][1]))
			return (ft_error(minishell, GENERAL_ERROR, ft_strjoin_gc("export: ", args[i], &minishell->gc), ": not a valid identifier"));
